...
- Homes persist on the chests themselves. A second press moves little or nothing, and established chests keep their role between runs. A category can still relocate when it no longer fits its chest.
...
- A chest another player is currently holding (by networked ownership) is skipped and retried instead of being taken over mid-browse. This closes the known issue from the 1.1.2 notes.
...
- Green `(N)` counts show what storage holds. Greyed out means there is nothing to fetch, because you already carry what the recipe needs or because storage has none of what is missing; it re-arms as crafting consumes materials.
...
- `sort: off` chests are now also safe from other chests' Pull buttons. Previously a neighbour's Pull could drain a chest that was marked off.
...
- The speed and UI knobs are each player's own now: `TransferInterval`, `StacksPerTick`, `MovesPerSecond`, `MaxMovesPerRun` and the `[Gather]` toggles are no longer admin-locked or server-synced. They only change how fast or what you see, never the outcome. A server's stored values for them no longer apply to clients.
